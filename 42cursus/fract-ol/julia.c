...
/*   Updated: 2023/09/30 21:41:16 by minwucho         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	rgb_julia_make(t_data *data)
{
	...
	data->g = ((double)(data->iter_num) / (2 * MAX_ITER)) * 255;
	data->b = ((double)data->iter_num / (3 * MAX_ITER)) * 255;
	...
	while (++y < HEIGHT)
	{
		...
		while (++x < WIDTH)
		{
			...
			if (data->iter_num == MAX_ITER)
				data->color = create_rgb(MAX_ITER, MAX_ITER, MAX_ITER);
			else
				rgb_julia_make(data);
